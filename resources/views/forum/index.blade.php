...
@section('masterDescription')
    Pantalla de bienvenida al ForoUG.
@endsection
@section('mainContent')
    <div class="row">
        <div class="col-md-3">
            @component('components.forumviewcomposer')
            @endcomponent
        </div>
        <!-- /.col -->
        <div class="col-md-9">
            <div class="row">
                <div class="col-lg-5 col-lg-offset-7">
                    <form method="GET" action="{{route('forum.index')}}" accept-charset="UTF-8"
                          class="header-search-wrapper ">
                        <div class="input-group content-group" style="margin-bottom: 10px !important;">
                            <div class="has-feedback has-feedback-left">
                                <input class="form-control input-xlg" placeholder="Buscar Comentarios" name="scope"
                                       id="scope" type="text" value="{{$scope}}">
                ...
                @foreach($comments as $comment)
                    <li id="liComment_{{$comment->id}}">
                        ...
                        <div class="timeline-item">
                            <span class="time" id="spantime_{{$comment->id}}"><i
                                        class="fa fa-clock-o"></i> {{$comment->updated_at->diffForHumans()}}</span>
                            <h3 class="timeline-header text-blue text-bold"
                                id="htime_{{$comment->id}}">{{$comment->fullname}}</h3>
                            <div class="timeline-body" id="bodytime_{{$comment->id}}">
                            ...
                            <div class="timeline-footer">
                                <div class="row">
                                    <div class="col-lg-6 col-md-6">
                                        ...
                                        @can('delete',$comment)
                                            <button class="btn btn-danger btn-xs" value="{{$comment->id}}"
                                                    onclick="deletePost(this)">Eliminar
                                            </button>
                                        @endcan
                                        @can('owner', $comment)
                                            <button class="btn btn-warning btn-xs" value="{{$comment->id}}"
                                                    onclick="updatePost(this)">Actualizar
                                            </button>
                                        @endcan
                                    </div>
                                    <div class="col-lg-6 col-md-6 text-right">

                                        <label onclick="like('{{$comment->id}}','F')" class="text-primary"><span
                                                    id="like_{{$comment->id}}">{{$comment->_like}}</span> <i
                                                    class="fa fa-thumbs-o-up text-brown-400  text-font-like"
                                                    style="font-size: 18px" aria-hidden="true"></i></label>&nbsp;
                                        <label onclick="desLike('{{$comment->id}}','F')" class="text-primary"><span
                                                    id="deslike_{{$comment->id}}">{{$comment->deslike}}</span> <i
            ...
            @if(count($comments)>0)
                <div class="col-lg-4 col-lg-offset-4">
                    <button class="btn bg-teal-600 btn-block" id="btn-more" data-id="{{$code}}"
                            data-categoria="{{$category}}"><i class="fa fa-plus"></i> VER MAS
                    </button>
                </div>
            @endif
        </div>
...
@section('masterJsCustom')
    ...
    <script>
        $(document).on('click', '#btnComment', function (e) {
            ...
            riot.mount('comment-form', {
                id: 0,
                action: 'guardar',
                categories: $('#hdfCategories').val(),
                'owner': '0'
            });
        ...
        $(document).on('click', '#btn-more', function () {
            ...
            var _categoria = $(this).attr('data-categoria');
            $(this).html('<i class="fa fa-repeat fa-spin"></i>  Loading....');
            var objApiRest = new AJAXRest("{{route('forum.index')}}", {
                id: _id,
                scope: $("#scope").val(),
        ...
        function deletePost(_btn) {
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        var objApiRest = new AJAXRest('/forum-comment/' + _btn.value + '/delete', {owner:'0',children:false}, 'GET');
                        objApiRest.extractDataAjax(function (_resultContent, status) {
                            if (status == 200) {
                                $('#ulCategories').html('');
                                $.each(_resultContent.categories,function(key,value){
                                    var code=0;
                                        if(!$.isEmptyObject(value.comments_count)){
