...
!function (e) {
    ...
    i = {
        ...
        constructor: t, _init: function (t) {
            ...
            var u, f = this, m = f.slug(a), g = "", v = f.previewSettings[e] || f.defaults.previewSettings[e], h = v && v.width ? v.width : "", w = v && v.height ? v.height : "", _ = d || f._renderFileFooter(m, l, i.isEmpty(h) ? "auto" : h, o), b = f._getPreviewIcon(a), C = b && f.preferIconicPreview, E = b && f.preferIconicZoomPreview, y = function (t, o, l, d) {
            ...
            r.loadedImages = [], r.totalImagesCount = 0, e.each(t, function (e, i) {
                ...
                var y, x, T, S, F, P, I, k, $, A = g + e, z = u + "-" + A, D = t[e], U = D.name ? r.slug(D.name) : "", j = (D.size || 0) / 1e3, R = "", L = i.objUrl.createObjectURL(D), M = 0, O = "";
                ...
                if (i.isEmpty(w) || (R = new RegExp("\\.(" + w.join("|") + ")$", "i")), T = j.toFixed(2), r.maxFileSize > 0 && j > r.maxFileSize)return F = r.msgSizeTooLarge.replace("{name}", U).replace("{size}", T).replace("{maxSize}", r.maxFileSize), void(r.isError = E(F, D, z, e));
            ...
            return i && (u = d._getLayoutTemplate("actionDelete").replace(/\{removeClass}/g, _).replace(/\{removeIcon}/g, h.removeIcon).replace(/\{removeTitle}/g, h.removeTitle).replace(/\{dataUrl}/g, c).replace(/\{dataKey}/g, p)), e && (f = d._getLayoutTemplate("actionUpload").replace(/\{uploadClass}/g, h.uploadClass).replace(/\{uploadIcon}/g, h.uploadIcon).replace(/\{uploadTitle}/g, h.uploadTitle)), t && (m = d._getLayoutTemplate("actionZoom").replace(/\{zoomClass}/g, h.zoomClass).replace(/\{zoomIcon}/g, h.zoomIcon).replace(/\{zoomTitle}/g, h.zoomTitle)), a && l && (s = "drag-handle-init " + h.dragClass,
                g = d._getLayoutTemplate("actionDrag").replace(/\{dragClass}/g, s).replace(/\{dragTitle}/g, h.dragTitle).replace(/\{dragIcon}/g, h.dragIcon)), v.replace(/\{delete}/g, u).replace(/\{upload}/g, f).replace(/\{zoom}/g, m).replace(/\{drag}/g, g).replace(/\{other}/g, w)
        }, _browse: function (e) {
            var i = this;
